...
| 设置         |                                                     |                                                            |
| ------------ | --------------------------------------------------- | ---------------------------------------------------------- |
| 识别模式     | `FAST_MODE` (默认) | `ACCURATE_MODE`                | 自由设置在脸部识别中的速度或者准确率。                     |
| 识别特征点   | `NO_LANDMARKS` (默认) | `ALL_LANDMARKS`             | 是否尝试去辨认脸上的特征点：眼睛，耳朵，鼻子，脸颊，嘴巴。 |
| 辨认脸部     | `NO_CLASSIFICATIONS` (默认) | `ALL_CLASSIFICATIONS` | 是否将脸分类成“睁开眼睛”或者“微笑”。                       |
| 脸部最小识别 | `float` (默认: `0.1f`)                              | 最小识别多大的脸部，与照片大小有关。                       |
| 允许人脸追踪 | `false` (默认) | `true`                             | 是否将人脸分ID，而后能用来进行追踪。                       |
...
for (FirebaseVisionFace face : faces) {
    ...
    if (face.getLeftEarPosition() != null) {
        PointF leftEarPos = face.getLeftEarPosition();
    }
    if (face.getNoseBasePosition() != null) {
